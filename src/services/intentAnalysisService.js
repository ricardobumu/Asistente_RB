...
class IntentAnalysisService {
  ...
  async analyzeMessage(message, context = {}, phoneNumber = null) {
    try {
      ...
      // Análisis con OpenAI
      const aiAnalysis = await this.analyzeWithOpenAI(message, context, preAnalysis);
      
      // Post-procesamiento y validación
      const finalAnalysis = this.postProcessAnalysis(aiAnalysis, preAnalysis, context);
      
      // Log del análisis (sin datos sensibles)
      logger.info("Message analysis completed", {
        intent: finalAnalysis.intent,
        confidence: finalAnalysis.confidence,
        hasPersonalData: finalAnalysis.entities.client_name || finalAnalysis.entities.phone,
  ...
  preAnalyzeWithKeywords(message) {
    ...
    analysis.hasPersonalData = phoneRegex.test(message) || 
                              emailRegex.test(message) ||
                              nameIndicators.some(indicator => lowerMessage.includes(indicator));

    // Detectar urgencia
    const urgencyWords = ['urgente', 'rápido', 'ya', 'ahora', 'hoy mismo', 'emergency'];
    analysis.urgencyIndicators = urgencyWords.filter(word => lowerMessage.includes(word));

    // Detectar sentimiento
    const positiveWords = ['gracias', 'perfecto', 'genial', 'excelente', 'encantada'];
  ...
  async analyzeWithOpenAI(message, context, preAnalysis) {
    try {
      ...
      const response = await openaiClient.chat([
        ...
        { role: "user", content: this.buildAnalysisPrompt(message, context, preAnalysis) }
      ], {
      ...
      if (response.choices[0].message.function_call) {
        return this.processFunctionCall(response.choices[0].message.function_call);
      } else {
        // Fallback a análisis tradicional
        return this.parseTraditionalResponse(response.choices[0].message.content);
      }
...
- booking_request: Solicitud de nueva reserva
- availability_inquiry: Consulta de disponibilidad
- booking_modification: Modificar reserva existente
- booking_cancellation: Cancelar reserva
...
- Respeta la privacidad y no inventes datos personales`;
  ...
  buildAnalysisFunctions() {
    return [
      {
        name: "analyze_intent",
        description: "Analizar la intención del mensaje y extraer información relevante",
        parameters: {
          type: "object",
          properties: {
            intent: {
            ...
            ready_to_book: {
              type: "boolean",
              description: "Si tiene suficiente información para hacer una reserva"
            },
            suggested_response: {
              type: "string",
          ...
          required: ["intent", "confidence", "entities", "missing_info", "ready_to_book"]
        }
      }
    ];
  }
  ...
  parseTraditionalResponse(content) {
    try {
      ...
      logger.error("Error parsing traditional response", { error: error.message });
      return this.createFallbackAnalysis();
    }
  }
  ...
  postProcessAnalysis(analysis, preAnalysis, context) {
    ...
    if (analysis.confidence > this.confidenceThresholds.HIGH && 
        preAnalysis.detectedServices.length === 0 && 
        analysis.intent === this.intentTypes.BOOKING_REQUEST) {
    ...
    const missingInfo = requiredForBooking.filter(field => !analysis.entities[field]);
    
    analysis.missing_info = missingInfo;
    analysis.ready_to_book = missingInfo.length === 0 && analysis.entities.client_name;
...
Puedes retirar tu consentimiento en cualquier momento escribiendo "STOP".`,
  ...
  createFallbackAnalysis() {
    return {
      ...
      ready_to_book: false,
      suggested_response: "Disculpa, no he entendido bien tu mensaje. ¿Podrías ser más específico sobre lo que necesitas? 😊",
  ...
  async analyzeSentiment(message) {
    try {
      const response = await openaiClient.chat([
        {
          role: "system",
          content: "Analiza el sentimiento de este mensaje en una escala de -1 (muy negativo) a 1 (muy positivo). Responde solo con el número."
        },
        {
          role: "user",
          content: message
