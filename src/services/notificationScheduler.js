...
class NotificationScheduler {
  ...
  async processScheduledNotifications() {
    try {
      ...
      for (const booking of upcomingBookings) {
        await this.scheduleBookingNotifications(
          booking,
          now,
          in24Hours,
          in2Hours
  ...
  async scheduleBookingNotifications(booking, now, in24Hours, in2Hours) {
    try {
      ...
      if (
        this.requiresPreparationInstructions(booking.service_name) &&
        appointmentTime <= in24Hours
      ) {
        await this.scheduleNotification(
          booking,
          client,
  ...
  async scheduleNotification(
    ...
    try {
      ...
      if (existingNotification.success && existingNotification.data) {
        logger.debug(
          `Notification ${notificationType} already sent for booking ${booking.id}`
        );
        return;
      }
      ...
      if (result.success) {
        ...
        logger.info(
          `📅 Scheduled ${notificationType} for booking ${booking.id}`
        );
      }
    } catch (error) {
    ...
    try {
      ...
      switch (notification.notification_type) {
        case "booking_reminder_24h":
          result = await notificationService.sendBookingReminder(
            booking,
            client,
            "24 horas"
          );
      ...
      logger.info(
        `📤 Notification ${notification.notification_type} ${status}`,
        {
  ...
  async sendPreparationInstructions(booking, client) {
    try {
      const instructions = this.getPreparationInstructions(
        booking.service_name
      );
      const message = this.buildPreparationMessage(
        booking,
        client,
        instructions
      );

      return await notificationService.sendWhatsAppMessage(
        client.phone,
        message
      );
    } catch (error) {
      logger.error("Error sending preparation instructions", {
        error: error.message,
      });
...
• Informa sobre problemas en los pies`,
    ...
    return (
      instructions[serviceKey] ||
      `• Ven puntual a tu cita
• Informa sobre alergias o sensibilidades
• Trae tu identificación`
  ...
  async getUpcomingBookings(fromDate, toDate) {
    try {
      const result = await this.appointmentModel.getUpcoming(fromDate, toDate);
  ...
  async processRetryQueue() {
    ...
    for (const item of itemsToRetry) {
      try {
        ...
        logger.info(
          `✅ Retry successful for notification ${item.notification.id}`
        );
      } catch (error) {
        logger.error(
          `❌ Retry failed for notification ${item.notification.id}`,
          {
        ...
        if (item.attempts < this.maxRetries) {
          item.nextRetry = new Date(
            Date.now() + this.retryDelay * item.attempts
          );
          this.retryQueue.push(item);
        } else {
          logger.error(
            `Max retries reached for notification ${item.notification.id}`
          );
          await this.notificationModel.updateStatus(
  ...
  async cleanupOldNotifications() {
    try {
      ...
      const result =
        await this.notificationModel.deleteOldNotifications(thirtyDaysAgo);
  ...
  /**
   * Programar recordatorios automáticos para una cita (método requerido por el controlador)
   * @param {string} appointmentId - ID de la cita
   */
  async scheduleAppointmentReminders(appointmentId) {
    try {
      logger.info("⏰ Programando recordatorios para cita", { appointmentId });

      // Obtener datos completos de la cita
      const appointmentResult =
        await this.appointmentModel.findById(appointmentId);
      if (!appointmentResult.success || !appointmentResult.data) {
        throw new Error(`Cita no encontrada: ${appointmentId}`);
      }
      ...
      if (appointmentTime <= now) {
        logger.warn("Cita es en el pasado, no se programarán recordatorios", {
          appointmentId,
      ...
      const clientResult = await this.clientModel.findById(
        appointment.client_id
      );
      if (!clientResult.success || !clientResult.data) {
        throw new Error(`Cliente no encontrado: ${appointment.client_id}`);
      }

      const client = clientResult.data;

      // Solo programar si el cliente tiene teléfono
      if (!client.phone) {
        logger.warn(
          "Cliente sin teléfono, no se programarán recordatorios WhatsApp",
          {
            appointmentId,
            clientId: client.id,
          }
      ...
      // Programar recordatorio 24h antes (si es en el futuro)
      if (reminder24h > now) {
        await this.scheduleNotification(
          appointment,
          client,
        ...
        logger.info("📅 Recordatorio 24h programado", {
          appointmentId,
          scheduledFor: reminder24h.toISOString(),
        });
      }

      // Programar recordatorio 2h antes (si es en el futuro)
      if (reminder2h > now) {
        await this.scheduleNotification(
          appointment,
          client,
        ...
        logger.info("📅 Recordatorio 2h programado", {
          appointmentId,
          scheduledFor: reminder2h.toISOString(),
        });
      }

      // Programar instrucciones de preparación (si el servicio lo requiere)
      const serviceName =
        appointment.services?.name || appointment.service_name || "";
      if (this.requiresPreparationInstructions(serviceName)) {
        const prepTime = new Date(
          appointmentTime.getTime() - 12 * 60 * 60 * 1000
        ); // 12h antes
        if (prepTime > now) {
          await this.scheduleNotification(
            appointment,
          ...
          logger.info("📋 Instrucciones de preparación programadas", {
            appointmentId,
            scheduledFor: prepTime.toISOString(),
          });
        }
      }
      ...
      logger.info("✅ Recordatorios programados exitosamente", {
        appointmentId,
        clientPhone: client.phone.substring(0, 5) + "***",
        appointmentTime: appointmentTime.toISOString(),
      });
    } catch (error) {
      logger.error("❌ Error programando recordatorios de cita", {
        appointmentId,
        error: error.message,
        stack: error.stack,
      });
