...
class ResponseGenerationService {
  ...
  async generateResponse(analysis, context = {}, availableServices = []) {
    try {
      ...
      let response = await this.generateBaseResponse(responseType, analysis, context);
  ...
  async generateBaseResponse(responseType, analysis, context) {
    switch (responseType) {
      case 'gdpr_consent_request':
        return analysis.suggested_response || this.getRandomTemplate('gdpr_consent');
  ...
  generateGreetingResponse(context) {
    ...
    const isReturningUser = context.previousMessages && context.previousMessages.length > 0;
  ...
  async generateBookingReadyResponse(analysis, context) {
    ...
    if (availabilityCheck.available) {
      ...
      return this.generateNoAvailabilityResponse(entities, availabilityCheck.alternatives);
    }
  }
  ...
  async generateAvailabilityResponse(analysis, context) {
    ...
    if (entities.service && entities.date) {
      const availability = await this.getAvailableSlots(entities.service, entities.date);
...
¿Sobre cuál te gustaría saber más detalles?`;
  ...
  generateDirectContactResponse(analysis, context) {
    ...
    if (analysis.entities.urgency === 'high') {
      return template + " He marcado tu consulta como urgente para que Ricardo te atienda lo antes posible.";
    }
  ...
  async generateComplaintResponse(analysis, context) {
    const empathyResponse = "Lamento mucho que hayas tenido una experiencia negativa. ";
    const actionResponse = "Voy a notificar inmediatamente a Ricardo para que se ponga en contacto contigo y resuelva esta situación. ";
    const followUpResponse = "¿Podrías contarme brevemente qué ha ocurrido para que pueda informarle mejor?";
  ...
  personalizeResponse(response, context) {
    ...
    if (context.sentiment === 'positive' && !response.includes('😊') && !response.includes('✨')) {
      response += ' ✨';
    }
  ...
  addContextualInfo(response, analysis, availableServices) {
    ...
    if (analysis.intent === 'general_inquiry' && availableServices.length > 0) {
      const serviceList = availableServices.slice(0, 3).map(s => s.name).join(', ');
  ...
  optimizeResponseLength(response) {
    ...
    if (response.length > maxLength) {
      ...
      response = response.substring(0, maxLength - 50) + "...\n\n¿Necesitas más información? 😊";
    }
