...
function auditResponse(req, res, data, requestInfo) {
  try {
    ...
    if (req.path.includes("/admin")) {
      ...
      auditAppointmentActivity(req, res, data, requestInfo, success);
 ...
 */
function auditAppointmentActivity(req, res, data, requestInfo, success) {
...
function auditWebhookActivity(req, res, data, requestInfo, success) {
  const webhookType = req.path.includes("whatsapp")
    ? "whatsapp"
    : req.path.includes("calendly")
      ? "calendly"
      : "unknown";
  ...
  if (Math.random() < 0.01) {
    setImmediate(() => {
      try {
        ...
        logger.error("Error in suspicious pattern detection", {
          error: error.message,
        });
      }
    });
